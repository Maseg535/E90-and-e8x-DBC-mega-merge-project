...
[void]$sb.AppendLine('.filter-wrap{display:flex;flex-wrap:wrap;gap:8px;margin:10px 0 0;}')
[void]$sb.AppendLine('.fbtn{padding:6px 14px;border-radius:999px;border:1px solid #475569;background:transparent;color:#94a3b8;font-size:13px;cursor:pointer;transition:background .15s,color .15s,border-color .15s;}')
[void]$sb.AppendLine('.fbtn.active[data-tier="0"]{background:#1e293b;color:#e2e8f0;border-color:#64748b;}')
[void]$sb.AppendLine('.fbtn.active[data-tier="1"]{background:#1e3a5f;color:#93c5fd;border-color:#3b82f6;}')
[void]$sb.AppendLine('.fbtn.active[data-tier="2"]{background:#14532d;color:#86efac;border-color:#22c55e;}')
[void]$sb.AppendLine('.fbtn.active[data-tier="3"]{background:#166534;color:#bbf7d0;border-color:#22c55e;font-weight:700;}')
[void]$sb.AppendLine('.fbtn:hover{border-color:#94a3b8;}')
...
[void]$sb.AppendLine('</div>')
[void]$sb.AppendLine('<div class="filter-wrap">')
[void]$sb.AppendLine('<button class="fbtn active" data-tier="0">Unknown</button>')
[void]$sb.AppendLine('<button class="fbtn active" data-tier="1">Unknown + values</button>')
[void]$sb.AppendLine('<button class="fbtn active" data-tier="2">Described</button>')
[void]$sb.AppendLine('<button class="fbtn active" data-tier="3">Fully known</button>')
[void]$sb.AppendLine('</div>')
...
foreach ($msg in $orderedMessages) {
    ...
    $msgHasKnown = $knownValues.ContainsKey([int]$msg.Id)
    $msgHasDesc  = $msg.Comment -or ($msg.Signals.Count -gt 0)
    $tier = if ($msgHasDesc -and $msgHasKnown) { 3 } elseif ($msgHasDesc) { 2 } elseif ($msgHasKnown) { 1 } else { 0 }
    [void]$sb.AppendLine("<section class='message' id='$anchor' data-search='$(Escape-Html $searchData)' data-tier='$tier'>")
...
[void]$sb.AppendLine('var fbtns=document.querySelectorAll(".fbtn");')
[void]$sb.AppendLine('function applyFilters(){')
[void]$sb.AppendLine('  var q=s.value.toLowerCase().trim();')
[void]$sb.AppendLine('  var active=new Set(Array.from(fbtns).filter(function(b){return b.classList.contains("active");}).map(function(b){return b.dataset.tier;}));')
[void]$sb.AppendLine('  msgs.forEach(function(m){')
[void]$sb.AppendLine('    var tierOk=active.has(m.dataset.tier);')
[void]$sb.AppendLine('    var searchOk=q.length===0||m.dataset.search.indexOf(q)!==-1;')
[void]$sb.AppendLine('    m.classList.toggle("hidden",!tierOk||!searchOk);')
[void]$sb.AppendLine('  });')
[void]$sb.AppendLine('}')
[void]$sb.AppendLine('s.addEventListener("input",applyFilters);')
[void]$sb.AppendLine('fbtns.forEach(function(b){')
[void]$sb.AppendLine('  b.addEventListener("click",function(){b.classList.toggle("active");applyFilters();});')
[void]$sb.AppendLine('});')
